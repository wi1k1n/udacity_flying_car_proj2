...
def create_grid(data, drone_altitude, safety_distance):
    ...
    offset = (int(north_min), int(east_min))
    return grid, offset, centers
...
def a_star_graph(graph, start, goal, h=None):
    """Modified A* to work with NetworkX graphs."""
    if not h: h = lambda n1, n2: LA.norm(np.array(n2) - np.array(n1))
    path = []
    path_cost = 0
    queue = PriorityQueue()
    queue.put((0, start))
